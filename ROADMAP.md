...
🟡 In progress on `feat/phase1-public-site` (built, not yet cut over). Done on the branch: Astro scaffold + pages (home, services, about, contact, history, privacy, book, guides, **service-area pages**), shared `BaseLayout` with per-page SEO + site-wide LocalBusiness JSON-LD, `@astrojs/sitemap`, a **care-guides content collection** (4 guides) carrying per-guide **FAQ + Article JSON-LD**, and a **service-area content collection** (5 towns) carrying **Breadcrumb + Service + FAQ JSON-LD** with the honest D-011 surcharge wording; the chrome rebranded from ASH-blue onto the ICC logo's green (L-010). Remaining: encode the surcharge figure once Mark confirms it (D-011), a home-page link to `/areas` + a mobile nav menu, more guides, robots review + Search Console/GBP, the review-request engine, and the cutover PR. Carry the Phase 0 hardening items below into go-live.
...
- [x] Area pages reflect the service-area model (D-011): Cheltenham and Gloucester as core/no-surcharge pages, plus wider-Gloucestershire pages (Stroud, Tewkesbury, Cirencester) that honestly state a small out-of-area surcharge applies. **Built** with a `tier` switch and the "confirmed at booking" placeholder — no figure invented. Still to do: confirm the exact surcharge figure and postcode boundary with Mark, then encode a concrete number on the pages + in `validateBooking`; add surrounding towns as wanted.

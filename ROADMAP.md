...
- [x] Stand up Supabase: relational DB + admin auth (hosted `icc-platform`, London). File storage for job photos: **built 19 September 2026 (D-047)**, the private `job-photos` bucket (migration `20260919120000`, to be applied to hosted before the push), written best-effort after the booking emails, read by the admin through one-hour signed URLs; the first hosted booking with a photo is the owed ride.
...
- [x] Job records: customer, address, carpet details, AI assessment, quote, slot, notes, and, since 19 September 2026 (D-047), the customer's photo in Storage with the admin card showing it (Mark still gets it by email and in the PDF as well).
- [x] Basic invoice tracking (draft / sent / paid / overdue) against completed jobs. **Built and live** (backend phase 1, 10 to 15 Sept 2026; D-026 supersedes the D-024 FreeAgent plan): platform-owned invoicing on Stripe (`invoices.js`, webhook, admin invoice panel), the accounting export (D-041) and a cash-basis P&L with an expense log (D-039/D-042). Dormant behind `STRIPE_SECRET_KEY` for invoicing. The first invoicing ride passed on 16 Sept 2026 (draft, send, pay, paid via the API reflection); the four `invoice.*` webhook events were subscribed on 17 Sept 2026 (read back via the API); the first paid-without-Refresh invoice ride is the remaining proof.
...
- [x] **Admin "Forgot password?" flow** (Ben, 16 Sept 2026, after a reset by script): a link on `admin.html` calling `resetPasswordForEmail`, a recovery handler on the same page that reads the token from the link and sets the new password, and the redirect URL allowlisted in Supabase Auth. **Built 18 Sept 2026 (D-046)** with the raw GoTrue REST the sign-in already uses, pinned in `test/admin-html-syntax.test.js`, and ridden end to end on the local stack; on hosted it works once Ben adds the admin origins to the Supabase Auth redirect allowlist ([docs/ADMIN_PASSWORD_RESET.md](docs/ADMIN_PASSWORD_RESET.md)). `scripts/admin-set-password.js` stays as the fallback.

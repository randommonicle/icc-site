...
| 4a | `feat/slice4-knowledge-source` | #16 | KB single source `shared/config/knowledge.js` (D-006) | **built, PR #16 open, gated** — byte-identical prompt, no live change |
| 4b | `feat/slice4b-escalate-tool` | #17 | `escalate_to_human` tool + server-side tool loop (D-019) | **built, PR #17 open, gated** — live-path change; stacked on #16 |
...
1. **Merge the Slice 4 PRs in order:** **#16** (4a — KB single source; byte-identical, no live change, safe to merge) then **#17** (4b — `escalate_to_human` + tool loop; **live-path change**, so build the Netlify deploy preview and check the assistant actually escalates on a damage-risk question and the operator email arrives, per L-012/L-004). #17 retargets to `main` once #16 merges. **#14** (cleanup script) stays open by choice; **#15 is merged**. Still settle two flagged assumptions with Mark (carried from Slice 3): the deposit base (assumed **inc-VAT**) and the `core_postcodes` boundary (D-011).
...
- **Open PRs (nothing merges without Ben's go-ahead):** **#16** Slice 4a (KB single source; byte-identical, no live change — safe). **#17** Slice 4b (`escalate_to_human` + tool loop; **live-path change**, stacked on #16, base retargets to `main` after #16 merges — verify on a deploy preview). **#14** `scripts/delete-booking.js` (chore — deliberately unmerged to avoid web-publishing it in Phase 0). Plus this docs/handover PR (off `main`). PR #15 (D-004 + D-019 + prior handover) and earlier #11/#12/#13 are **merged + live**.

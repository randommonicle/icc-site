...
**RED finding, read this first (19 Sept, verified by two independent reads).** Public sign-ups are **ENABLED** on the hosted Supabase project (`disable_signup: false` from the Management API `GET /v1/projects/qzcfgpfvzpynnjgriqqn/config/auth`, and again from GoTrue's public `/auth/v1/settings` with the publishable key), against the "public sign-ups disabled" recorded on 15 June 2026 and repeated in `adminAuth.js`, `admin.html`, `.env.example`, CLAUDE.md, D-046 and `docs/ADMIN_PASSWORD_RESET.md`. Nothing exposed: `requireAdmin` checks `ADMIN_EMAILS` on every request, every public table has RLS on with no policies (nine tables, checked on the local schema which carries the same 13 migrations), `operator_admit` is revoked from `anon`/`authenticated`, no Storage buckets exist. What a stranger CAN do today: create an account in the ICC auth project (email confirmation required, the built-in mailer capped at 2/hour), which also breaks the "planted recovery fragment is inert" argument in D-046 (a stranger's link would put THEIR session on the recovery card; the operator's own account is untouched, no privilege gain). **The fix is one toggle** (Supabase dashboard, Authentication, Sign In / Providers, "Allow new users to sign up" OFF) **or one command** on Ben's yes, which also does the first action: `node scripts/supabase-auth-config.mjs --apply --disable-signups`. Not done here: it is a production config change and the brief said no deploys. Read-back guard: `ICC_HOSTED_IT=1 node --test test/hosted-auth-settings.test.js` (red today by design, green after the flip). Recorded: D-046 addendum (19 Sept), **L-044 drafted for Ben's acceptance**, every stale claim corrected in place (commit `7231202`). Whether the toggle was never set in June or was reset later is unknowable from here.
...
- CP-1 (`598065e`): the start-up verification above; no code changed yet.
- CP-2 (`7231202` + this commit): **the first action scripted and the GET half done.** `scripts/supabase-auth-config.mjs` (dry run / `--apply` / `--replace` / `--disable-signups` / `--site-url`), 12 unit tests with a fake transport (two mutations proven red), `test/hosted-auth-settings.test.js` (guarded, red until the flip), `docs/ADMIN_PASSWORD_RESET.md` owner step 1 rewritten as Route A (scripted) and Route B (dashboard), `.env.example` documents the token. Live dry run against the real project (verified): `site_url` = `http://localhost:3000` (the default, never set), `uri_allow_list` empty, **`disable_signup` = false (the RED finding above)**, `external_email_enabled` true, `mailer_autoconfirm` false, `mailer_otp_exp` 3600, `password_min_length` 6, `rate_limit_email_sent` 2, built-in mailer (no SMTP host), recovery subject "Reset your password", recovery template customised and carrying `{{ .ConfirmationURL }}`. So of the four settings the 18 Sept entry listed as unreadable, three are now read (OTP 3600 s OK, template OK, Site URL is the default and needs the PATCH) and the fourth is the finding. Suite 618 tests, 605 pass, 0 fail, 13 skip (the new hosted guard is the thirteenth). Usage at CP-2: 5-hour 20%, weekly 74% (+1 for this unit), context 25%.
- CP-3 (this commit, docs only): **the unproven `/admin` rewrite seam is now proven locally (verified, in-app browser).** `netlify dev --dir site/dist --offline` on port 8899 applies `netlify.toml`'s `/admin` to `/admin.html` `status = 200` rewrite exactly as production does (the root `.env` was moved aside for the run so the loaded functions carried no production keys; `admin.html` copied into `site/dist` as the build command does, removed afterwards with the `deno.lock` the CLI wrote). `http://localhost:8899/admin#access_token=junk…&type=recovery` landed on `/admin` with `location.hash === ""`, the sign-in card hidden, the recovery card shown and focus on the new-password field, and the network log showing exactly one request (`GET /admin` 200): `initPasswordRecovery()` makes no network call, so the junk token never reached any GoTrue. The error shape too: `#error=access_denied&error_code=otp_expired&…` (after a real reload; a fragment-only navigation does not re-run the page) showed the sign-in card with "That reset link has expired or was already used. Request a new one." and the hash wiped. What the hosted ride still owns is only GoTrue's side (the real link, the real session): the page-side seam is done. The 18 Sept "Verification still outstanding" third bullet is closed by this.
- CP-4 (`dc6ea92`, `59b3ecc`, `9a8335a` + this docs commit): **job photos into Supabase Storage, `TODO(slice5x/photos)` resolved (D-047, drafted for Ben).** Plan and checkpoint log in `docs/JOB_PHOTOS_PLAN.md` (advisor-reviewed before the first commit). Migration `20260919120000_job_photos_bucket.sql` (private `job-photos` bucket, 4 MB, the four validated types; catalog-verified on the local stack; pgTAP 9/9 proven red), `jobPhotoStore.js` (server-derived path, upload-then-row, never throws, 8 s deadline, batch signing, erasure; 8 unit + the `[integration]` round trip with the public-path and text/html refusals as negative controls), the wiring (`chat.js` stores at every success return AFTER both emails, pinned and proven red by moving it; `bookingsStore.js` joins `job_photos`, resolved on real PostgREST; `bookings.js` signs one batch; `admin.html` renders only a `SUPABASE_URL + "/storage/v1/"` url, pinned and proven red). Suite **631 tests, 617 pass, 0 fail, 14 skip**; integration 133/133 in three consecutive runs (one unidentified failure in the very first integration run after the join landed, not reproduced in four runs since; if a race test flakes again, name it). **TWO THINGS THIS CHANGES FOR THE MERGE:** (a) the 18 Sept merge sheet's "no migration is pending" is no longer true: `bash scripts/db-push.sh --dry-run` will list `20260919120000`, and `bash scripts/db-push.sh` must apply it to hosted BEFORE the push or the D-043 hook stops the push (on Windows: `& "C:\Program Files\Git\bin\bash.exe" scripts/db-push.sh`); (b) `scripts/supabase-auth-config.mjs` and `jobPhotoStore.js` are NOT on the CLAUDE.md scripts line / tree where the launch-check branch edits, deliberately (the tree line for `jobPhotoStore.js` was added under `bookingsStore.js`, which neither launch branch touches); add the script to the scripts line after the launch branches merge. Usage at CP-4: 5-hour 40%, weekly 76% (+2 for the slice), context 36%.
- CP-5 (`41c1cc9`, `4d951c8` + this docs commit): **cross-agent review of the photos slice, converged; five findings fixed, one residual accepted; `scripts/erase-job.js` built.** GPT via codex, three rounds, read-only sandbox on this worktree (`exchange/REVIEW_job-photos_2026-09-19.md`, machine-local; seat spend 0.86M / 1.9M / 2.7M gross input, mostly cache reads). Fixed: (1) a deadline cannot cancel an in-flight call, so the order is now ROW first, object second (the only reachable inconsistent state is a row without an object, never bytes no erasure can find), ONE 5 s deadline for the whole store call, cleanup never awaited, late results tidy their own row; (2) the upload undo removes the object BEFORE the row and only drops the row when that succeeded, because storage-js reports a committed object with an unparsable success body as an error (`{ data: [] }` for a missing key measured on the local stack); (3) signing bounded (5 s); (4) the admin guard is the exact `SUPABASE_URL + "/storage/v1/object/sign/job-photos/"` prefix; (5) the bucket migration is corrective (`on conflict do update`, proven by hand-breaking the local bucket and re-running the file); (6) **`scripts/erase-job.js`**, the jobs half of the Art.17 procedure: report mode read-only, `--delete` erases the photo objects and rows before the job row and REFUSES an invoiced job (financial record), `--with-customer` removes the customer only when unreferenced; `test/erase-job.test.js` unit + an `[integration]` erase of a real stored photo. Accepted residual (recorded in the script header and D-047): the invoice check and the job delete are two statements, so an invoice created between them leaves the photos erased and the invoiced job kept, an acceptable and exactly-reported end state. D-047 rules 3 and 5 rewritten to the reviewed shape. Suite **637 tests, 622 pass, 0 fail, 15 skip**; both integration files green. Usage at CP-5: 5-hour 61%, weekly **79%** (+3 for the review cycle), context 47%. **The 85% no-new-work line is close: after this checkpoint only closing work (handover, memory) or one small item.**

**Ben's merge sheet (PowerShell, from the MAIN checkout `C:\Users\bengr\Projects\ICC\icc-site`; every step is yours, nothing here was pushed).** The order from the 18 Sept entry stands, with this branch in the hygiene branch's place and ONE hosted migration now pending.
1. Review the diff: `git log --oneline main..claude/unattended-work-session-798e3e` (24 commits: the 12 hygiene-branch commits, then `598065e` … the tip) and `git diff main..claude/unattended-work-session-798e3e --stat`.
2. Decide the two drafts: L-044 (LESSONS_LEARNED.md) and D-047 (DECISIONS.md); each is one revert if unwanted.
3. Your yes, then the hosted auth config in one command from the main checkout (the token is in the shell environment): `node scripts/supabase-auth-config.mjs` (dry run, read it), then `node scripts/supabase-auth-config.mjs --apply --disable-signups`, then `$env:ICC_HOSTED_IT="1"; node --test test/hosted-auth-settings.test.js; Remove-Item Env:ICC_HOSTED_IT` (green = sign-ups off).
4. `git checkout main; git merge --ff-only claude/launch-check` (independent, ready).
5. `git checkout claude/unattended-work-session-798e3e; git rebase main` (one command; the only shared file with launch-check is CLAUDE.md's scripts line, which this branch did not touch, so no conflict expected), then `git checkout main; git merge --ff-only claude/unattended-work-session-798e3e`.
6. **The hosted migration BEFORE the push (L-040, the D-043 hook refuses otherwise):** `& "C:\Program Files\Git\bin\bash.exe" scripts/db-push.sh --dry-run` (expect `20260919120000_job_photos_bucket.sql` listed), then `& "C:\Program Files\Git\bin\bash.exe" scripts/db-push.sh`, then the four catalog queries in the migration file against hosted (Supabase SQL editor or `psql`): the bucket row `f | 4194304 | the four types`, `storage.objects` RLS `t`, 0 policies, 0 objects.
7. `npm run build --prefix site; node --test` (expect 637 / 622 / 0 fail / 15 skip); the tip must not be `[skip ci]` (the wrap commit of this run, the branch tip, is deliberately NOT `[skip ci]`; if you add docs commits on top, keep the last one deploying, L-037), then `git push origin main`.
8. After the deploy: the one real ride for D-047 (a booking WITH a photo on the `.netlify.app` host: the admin card shows it, the function log carries `job photo stored: jobs/<id>/photo-….jpg`), the hosted password-recovery ride (18 Sept list), the D-045 cold ride.
9. `claude/launch-flip-noindex` only after the domain resolves and the privacy reading: `git checkout claude/launch-flip-noindex; git rebase main` then ff-merge, set `PUBLIC_SITE_URL` and `ALLOWED_ORIGINS`, redeploy, `node scripts/launch-check.mjs https://www.intelligentclean.co.uk`.
10. Housekeeping: `git worktree remove` this worktree and the two launch worktrees once merged; delete the branches; add `supabase-auth-config.mjs` and `erase-job.js` to the CLAUDE.md scripts line (deliberately not done here, that line is the launch-check branch's).

**Wrap (CP-6, this commit, deliberately NOT `[skip ci]` so the batch's tip carries the deploy, L-037).** The run stopped on the usage line, not on a lack of work: weekly at the reading below, the 85% no-new-work rule bit after CP-5. Final verification at the tip (verified): `supabase db reset` clean; pgTAP **104/104** (9 files); every `[integration]` file on the local stack **139/139** (with `ICC_SUPABASE_IT=1` and the `status -o env` keys); `node --test` with `site/dist` built **637 tests, 622 pass, 0 fail, 15 skip** (14 `[integration]` cases that need the local stack, 1 `[hosted]` read-back; the two `[build]` guards run because `site/dist` is built); `npm audit --omit=dev` at the root: 0 vulnerabilities. Tree clean. **Branch: 24 commits ahead of `main`** (the 12 hygiene-branch commits, then 12 of this run: 6 code, 6 docs including this one).

**What this run did NOT do, and why.** No push, no deploy, no hosted write (the brief). No PATCH of the hosted auth config (needs your yes; the dry run is in CP-2). No D-045 cold ride and no hosted password-recovery ride (an admin login is needed). No `npm audit fix` in `site/` (see the toolchain note below; a lockfile change is yours to review). No CLAUDE.md scripts-line edit (the launch-check branch owns that line).

**New items for your list (found, not fixed).**
- **Site toolchain advisories** (`cd site; npm audit --omit=dev`): astro 5.18.2 carries a published critical (XSS in `define:vars`; server-island parameters) and high advisories via sharp/libvips, svgo, postcss, js-yaml, nanoid, smol-toml; the fix for astro is the 7.x major. **Exposure today is low:** the site is a static build, `grep -rn "define:vars\|server:defer" site/src` finds nothing, and the rest are build-time tools processing the repo's own trusted inputs. Two-step: `npm audit fix` (non-breaking, for the minor ones; check the lockfile diff and rebuild) now; the astro 7 major bump as its own attended unit under `dependency-upgrade-verification` (the build guards `[build]` and the SEO-limits tests are the safety net).
- The privacy notice's Supabase line could say "records, including any photographs" now that photos are held there (optional; the existing text already names photographs as data collected and Supabase as the processor; D-047 note 6).
- One unidentified integration failure in the very first run after the `job_photos` join landed (CP-4), not reproduced in five clean runs since; if a race test flakes again, name it.

**Traps / working agreements (this run).**
- The Bash tool strips backslashes in `node -e '…'` command strings AND inside some heredocs: a `\\/` in a regex became `/` (a syntax error caught by the test run), a `\n` in an anchor string became a literal and the mutation silently did not apply (reported as green when nothing had changed). Rule reaffirmed: any edit carrying a backslash goes through the Edit or Write tool, and a mutation test is proven by the RED run, never by the green one.
- A `Promise.resolve(builder)` on a supabase-js thenable executes the query once; attaching a continuation and an `await` to that one promise is safe (GPT round 2 measured one execution).
- `netlify dev --offline` still LOADS the functions from `.env`: move the file aside for a page-only ride; it writes `deno.lock` at the root (delete it) and needs `admin.html` copied into `site/dist` as the build command does.
- The Supabase Management API's GET answers 243 auth fields including secrets (`smtp_pass`, hook and provider secrets): never print the raw response; the script prints a named allowlist only.
- `npm approve-scripts esbuild` writes an `allowScripts` block into `site/package.json`; revert it, never commit it.

**Citations for this entry** (quoted text = the line as read at the tip; "How verified" = the command run).

| Claim | Path | Line | Quoted text | How verified |
|---|---|---|---|---|
| The one-command owner step | `scripts/supabase-auth-config.mjs` | 36 | `export const PROJECT_REF = "qzcfgpfvzpynnjgriqqn"; // icc-platform, London (D-009 addendum)` | `grep -n` |
| The refusal | `scripts/supabase-auth-config.mjs` | 126 | `  if (config.disable_signup !== true) return \`sign-ups are NOT disabled on the hosted project …` | `grep -n` |
| The hosted read-back | `test/hosted-auth-settings.test.js` | 29 | `test("[hosted] sign-ups are disabled and the email provider is on (GoTrue /auth/v1/settings)", {` | `grep -n` |
| Row first | `server/netlify/functions/jobPhotoStore.js` | 113 | `    // 1. The row, first (rule 3). A row that commits after the deadline has no upload` | `grep -n` |
| The undo, object then row | `server/netlify/functions/jobPhotoStore.js` | 106 | `  const undoPair = (id, why) => fireAndForget(async () => {` | `grep -n` |
| One deadline for the call | `server/netlify/functions/jobPhotoStore.js` | 155 | `    return await withDeadline(run(), deadlineMs, "job photo store", () => { state.expired = true; });` | `grep -n` |
| The store call after the emails | `server/netlify/functions/chat.js` | 1177 | `    await storePhotoBestEffort();` | `grep -n` (three call sites: 978, 1177, 1202) |
| The join | `server/netlify/functions/bookingsStore.js` | 260 | `…,customers(name,phone,email),job_photos(id,storage_path,media_type,created_at)"` | `grep -n` |
| Batch signing | `server/netlify/functions/bookings.js` | 97 | `      pgBookings = await signPhotoUrls(supabase, pgBookings);` | `grep -n` |
| The admin guard | `admin.html` | 1114 | `  const photoUrlSafe = (b.photo && typeof b.photo.url === "string" && b.photo.url.startsWith(SUPABASE_URL + "/storage/v1/object/sign/job-photos/")) ? b.photo.url : null;` | `grep -n` |
| Corrective bucket upsert | `supabase/migrations/20260919120000_job_photos_bucket.sql` | 29 | `on conflict (id) do update` | `grep -n` |
| Erasure refusal | `scripts/erase-job.js` | 87 | `  if (report.invoices > 0) return { ok: false, refused: …` | `grep -n` |
| D-047 | `DECISIONS.md` | 651 | `## D-047 — Job photos live in a private Supabase Storage bucket, …` | `grep -n` |
| L-044 | `LESSONS_LEARNED.md` | 189 | `## L-044 — A security setting recorded as "done" in the docs was never read back; …` | `grep -n` |
| Hosted sign-ups state | hosted GoTrue | `/auth/v1/settings` | `{"disable_signup":false,"external_email":true,"mailer_autoconfirm":false,…}` | `curl` with the publishable key, 19 Sept |
| This run's commits | git | `598065e` … tip | `docs: 2026-09-19 checkpoint CP-1 …` through this commit | `git log --oneline main..HEAD` (24) |
| Suite at wrap | `node --test` | summary | `tests 637`, `pass 622`, `fail 0`, `skipped 15` | `node --test` |
...
- `1bb7350` docs: **L-043 drafted** (the binary-file lesson). Ben: keep (agreed later the same evening).
...
3. ~~**Whether to take the "Forgot password?" flow** at all; it was built as droppable, not as approved (no answer came during the session).~~ **RESOLVED later the same evening: Ben, "yes to the password flow, I agree with all your suggestions". The whole branch is taken; the Supabase allowlist step and the hosted ride are the follow-ups.** The paragraph below is kept for the record of why the boundary was not clean. The four flow commits (`598f2e9`, `a3948b8`, `e4aaaa4`, `dbe4b81`) are NOT a clean boundary to stop under: `dbe4b81` also carries the `test/email-identity.test.js` hardening (word boundaries, the literal `from:`/`reply_to:` pattern, the wider sweep), which belongs to the identity refactor, and the handover sits above them. So: take the whole branch either way; if the flow is unwanted, the next session reverts the flow hunks (`admin.html`, `test/admin-html-syntax.test.js`'s recovery pin, `test/admin-recovery-behaviour.test.js`, `scripts/ride-admin-recovery-local.mjs`, `docs/ADMIN_PASSWORD_RESET.md`, the D-046 record, the ROADMAP tick, `supabase/config.toml`'s allowlist line) with a note, keeping the identity-test hunk of `dbe4b81`. If yes: the Supabase allowlist step in `docs/ADMIN_PASSWORD_RESET.md`, then the hosted ride above.
4. ~~**L-043 and D-046** are drafts for Ben's acceptance (the global rule is to prompt, not to write unilaterally; they are on the branch so the veto is one revert).~~ **RESOLVED: both accepted by Ben later the same evening; D-046's status line updated.**
5. The rest of the 17 Sept list #5 (Resend account, Search Console, GBP, the two owner copy calls) is unchanged.

**Next actions (ordered).**
**FIRST ACTION NEXT SESSION (Ben decided, 18 Sept late evening): set the hosted Supabase auth config for the password flow, through the Management API, before anything else.** **(19 Sept: SCRIPTED and the GET half DONE, see the 19 Sept entry; the manual curl steps below are superseded by `node scripts/supabase-auth-config.mjs`, and the dry run found sign-ups ENABLED on hosted, so the command to run on Ben's yes is `--apply --disable-signups`.)** Established this evening (verified unless marked):
- One Supabase account, two orgs: "randommonicle's Org" (PropOS, the inspection app) and "Intelligent Carpet Cleaning" (`icc-platform`, ref `qzcfgpfvzpynnjgriqqn`, London). The CLI on this machine is logged in as that account and lists all three projects (`npx supabase projects list`; an earlier eight-line truncation of that listing hid the ICC row and briefly misled the session). The repo is NOT linked to the project (`supabase/.temp/project-ref` absent) and does not need to be for this.
- The CLI has no command for individual auth settings; `supabase config push` sends the whole local `config.toml` auth section and has no dry run in 2.105.0 (installed) or 2.117.0 (newest, checked): pushed as it stands it would switch sign-ups on, set the Site URL to `127.0.0.1` and drop the per-address email limit to one second. Not the route tonight; a reviewed `[remotes]` production override block could make it the durable route later (anchor: `TODO(supabase/config-push-remotes)` in `supabase/config.toml`).
- The CLI's login token sits in Windows Credential Manager and is not to be dug out; the surgical route is the Management API with a personal access token.

The steps, in order:
1. **Ben:** supabase.com, account menu, Access Tokens, generate a token (name it, e.g. `icc auth config`), and add one line to the local `.env` in the repo root: `SUPABASE_ACCESS_TOKEN=` followed immediately by the token (no space; never into chat; `.env` is git-ignored). Say go.
2. **Session:** read the token into a shell variable without printing it (the same pattern as the Netlify and Stripe reads on 17 Sept). `GET https://api.supabase.com/v1/projects/qzcfgpfvzpynnjgriqqn/config/auth` with `Authorization: Bearer <token>`; show Ben the current `site_url`, `uri_allow_list`, `disable_signup`, `mailer_otp_exp` and whether the recovery template (`mailer_templates_recovery_content`, if returned) still carries `{{ .ConfirmationURL }}`. Field names are from memory of the Management API and are UNVERIFIED until that GET returns; take the names the GET shows.
3. **Session, on Ben's explicit yes to the exact body:** `PATCH` the same endpoint with ONLY two fields: `site_url` = `https://super-frangollo-c3a14a.netlify.app` (becomes `https://www.intelligentclean.co.uk` at the cutover) and `uri_allow_list` = the four admin URLs from `docs/ADMIN_PASSWORD_RESET.md` owner step 1, comma-separated (`https://super-frangollo-c3a14a.netlify.app/admin`, `https://www.intelligentclean.co.uk/admin`, `https://intelligentclean.co.uk/admin`, `https://*--super-frangollo-c3a14a.netlify.app/admin`; if the API rejects the wildcard entry, drop it and note it). Then `GET` again and quote the two values back. If `disable_signup` is false, STOP and tell Ben before anything else: sign-ups being off is what keeps a stranger's planted recovery fragment inert on the admin page.
4. Ben revokes the token on the same dashboard page, or keeps it for the cutover; record which in the next handover. Then the hosted password-recovery ride (Verification still outstanding, above) once the branch is merged and deployed.

**Next actions (ordered, after the first action).**
1. Ben: the three items above (domain, privacy reading; the password flow is now decided: take the whole branch).
...
- **Do not reason from a truncated listing.** `npx supabase projects list | head -8` cut the table one row short and the session concluded for a whole exchange that the CLI could not see `icc-platform`; the full listing shows it. A sweep whose output was cut is not evidence of absence (the blast-radius-grep rule, bitten live).

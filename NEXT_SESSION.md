...
1. **Merge the two open docs/chore PRs** — **#15** (docs: D-004 addendum + D-019 + this handover; merging makes the handover live on `main`) and **#14** (the cleanup script — optional, web-publish caveat). **Slice 3 quote foundation is merged + live** (#13): `isOutOfArea` + `quote` server-side enforcement (D-011), the `shared/contract/` versioned types (D-003), and the stateless `/api/v1/quote` endpoint. **Remaining Slice 3 (pairs with Slice 5):** the rest of `/api/v1/*` — Postgres-backed `/api/v1/bookings` + `availability` and Supabase Auth/admin RLS — which write/duplicate live behaviour. Settle two flagged assumptions with Mark: the deposit base (assumed **inc-VAT**) and the `core_postcodes` boundary (D-011).
2. **Slice 4: backend AI** through the `messages` table (draft to approve to send): photo-assessment re-run, invoice interpretation + chasing, summaries, NL admin queries (read-only/whitelisted), re-engagement (consent-gated, Phase 4). **Also the home of the D-019 grounding/escalation work** — the `escalate_to_human` handoff is a `messages` draft for Mark to approve, and the Citations-grounded KB is the D-006 build-out.
...
- **Open PRs (nothing merges without Ben's go-ahead):** **#15** docs (D-004 payments addendum + D-019 AI grounding + this handover refresh — **merge to make this handover live on `main`**; docs-only, safe). **#14** `scripts/delete-booking.js` (chore — deliberately unmerged to avoid web-publishing it in Phase 0; merge or leave). This session's #11 (sweep), #12 (deposit guard), #13 (Slice 3 quote) are already **merged + live**.
...
- **AI grounding (D-019):** strategy agreed; build with Phase 1 knowledge consolidation + Slice 4. Still need Mark's confirmation on the **deposit base (assumed inc-VAT)** and the **`core_postcodes` boundary** (esp. whole-GL54 vs Winchcombe-only — GL54 also covers far Cotswold villages).

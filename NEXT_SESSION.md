...
| 3 | `feat/slice3-server-pricing` | #13 | `/api/v1/quote` + server-side pricing/surcharge (D-003/D-011/D-012) | **quote foundation in review**; Postgres bookings/availability + Auth → Slice 5 |
...
1. **Review/merge PR #12 (deposit guard) + PR #13 (Slice 3 `/api/v1/quote`)** — both off `main`, `node --test` green, no live change. **Slice 3 quote foundation is done** (PR #13): `isOutOfArea` + `quote` server-side enforcement (D-011), the `shared/contract/` versioned types (D-003), and the stateless `/api/v1/quote` endpoint. **Remaining Slice 3, pairs with Slice 5:** the rest of `/api/v1/*` — Postgres-backed `/api/v1/bookings` + `availability` and Supabase Auth/admin RLS — which write/duplicate live behaviour. Settle two flagged assumptions with Mark: the deposit base (assumed **inc-VAT**) and the `core_postcodes` boundary (D-011).

...
1. **Ben's go, then push `claude/operator-background-turn` to `main`** (15 commits, fast-forward onto `30bd902`; the tip is a docs commit with no `[skip ci]`, so the build runs; push to main deploys; the migration is already on hosted, so the hook passes). Confirm the deploy `ready` via the API; a POST to the live `/.netlify/functions/operatorTurn-background` with a junk uuid should answer 202 with an empty body.

...
1. **Ben's go, then push `claude/operator-background-turn` to `main`** (12 commits, fast-forward; push to main deploys; the migration is already on hosted, so the hook passes). Confirm the deploy `ready` via the API; a POST to the live `/.netlify/functions/operatorTurn-background` with a junk uuid should answer 202 with an empty body.
...
6. Housekeeping when both agent branches have merged: `git worktree remove` the two worktrees and delete the branches; the runbook's stale line 170 and `netlify.toml`'s "parking page" comment go with the flip.

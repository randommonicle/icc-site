...
- [ ] **Set `ALLOWED_ORIGINS`** in Netlify. The chat origin check fails open by default; the rate limits are currently the only defence on the AI cost path (L-001). Blocked on the final domain (D-013); the code auto-enforces strict mode once the env var is set (`chat.js`). **Safe to enable now (19 June 2026):** strict mode also trusts the site's own Netlify deploy origins, so setting it no longer 403s the `.netlify.app` host or deploy previews (L-001 addendum). It stays defence-in-depth, not the primary control — a no-`Origin` scripted request still bypasses it, so the per-IP rate limit remains the real defence.

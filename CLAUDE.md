...
- [ ] **Turn on review requests (D-025).** The one-click review flow is built but dormant. Before it sends anything: set **`GOOGLE_REVIEW_URL`** (Mark's Google Business Profile review link — a short `g.page/r/...` keeps the SMS to one segment), **`SMSWORKS_API_KEY`** (from Mark's The SMS Works account, D-009), and optionally **`SMS_SENDER_ID`** (default `ICCleaning`) in Netlify. Email also relies on the A4 vars (`CUSTOMER_FROM`, `CUSTOMER_REPLY_TO`, `PUBLIC_SITE_URL`). No deploy needed — the env vars flip it on.

...
- **Hours, slots, pricing, deposit:** all in the `STATIC_SYSTEM_PROMPT`. Pricing is flat — Mark is not VAT-registered, so no VAT is added (the listed price is what the customer pays); see D-024. A 10% non-refundable deposit secures a slot.

...
- **Service area (D-011):** Cheltenham, Gloucester and Winchcombe are core (no travel charge). Everywhere else in Gloucestershire (Stroud, Tewkesbury, Cirencester and the surrounding GL towns) carries a **flat £15 out-of-area surcharge** (confirmed by Mark, June 2026). The assistant quotes the £15 concretely and includes it in the itemised quote for out-of-area addresses. Remaining: the precise postcode boundary for "out of area" and **server-side enforcement** — `validateBooking` currently only bounds the total price (£30–£5000), so the surcharge lives in the assistant's quote, not the server; encode the boundary + surcharge in `validateBooking` in Phase 2 (pairs with moving pricing/area logic server-side, D-007).
- **Hours, slots, pricing, deposit:** all in the `STATIC_SYSTEM_PROMPT`. Pricing is flat — Mark is not VAT-registered, so no VAT is added (the listed price is what the customer pays); see D-024. A 10% deposit secures a slot; the deposit/cancellation and re-clean wording is single-sourced in `shared/config/policy.js` (consumed by the prompt, the confirmation email and /terms).
...
- **Admin auth is per-user Supabase Auth (Slice 5d, live).** The operator signs in with email + password via the Supabase Auth password grant; the session access token is held in memory only (cleared on logout) and sent as `Authorization: Bearer <jwt>`. `adminAuth.requireAdmin` verifies the JWT (`auth.getUser`) and checks the `ADMIN_EMAILS` allowlist on top of the project's sign-ups being disabled (an owner setting: found ENABLED on 19 Sept 2026 and read back since by `test/hosted-auth-settings.test.js` with `ICC_HOSTED_IT=1`, L-044). The legacy `ADMIN_SECRET` is no longer read by the app. A "Forgot password?" link on the sign-in card runs Supabase Auth's email recovery through the same raw GoTrue REST (D-046; the owner steps, the redirect allowlist above all, are in [docs/ADMIN_PASSWORD_RESET.md](docs/ADMIN_PASSWORD_RESET.md)); `scripts/admin-set-password.js` remains the operator-run fallback.
...
### Post-job review requests (`server/netlify/functions/reviewRequest.js` + `admin.html`)
One-click **"mark complete & request review"** on each admin booking card (D-025). It sets `jobs.status='completed'` and sends a Google-review ask to the customer over **email (Resend)** and **SMS (The SMS Works, via the provider-agnostic `smsProvider.js`)**, logging one `messages` row per channel. Mirrors the handoff discipline: human-gated (only Mark's click sends), **fail-closed** (a channel is logged `sent` only after the provider accepts; a failure never blocks the other channel), **idempotent** (a channel already sent is skipped unless `resend:true`), and **transactional** (review_request has no consent gate — the schema's own note; contrast the D-008 consent-gated reengagement). Pure content builders live in `shared/reviewMessages.js`; the review link + SMS sender come from `shared/config/reviews.js`. **Dormant until configured:** with `GOOGLE_REVIEW_URL` unset the job still completes but nothing is sent; with `SMSWORKS_API_KEY` unset only the email goes. See the pre-launch checklist for the env vars.
...
- [x] **Set `ALLOWED_ORIGINS`** in Netlify. The chat origin check fails open by default; the rate limits are currently the only defence on the AI cost path (L-001). **Done (30 July 2026):** set via the Netlify API to `https://intelligentclean.co.uk,https://www.intelligentclean.co.uk`; strict mode live-verified the same day (hostile origin → 403, own `.netlify.app` origin still 200 — strict mode trusts the site's own Netlify deploy origins too, L-001 addendum). It stays defence-in-depth, not the primary control — a no-`Origin` scripted request still bypasses it, so the per-IP rate limit remains the real defence.
...
- [x] **Handoff-lead retention (review finding A5, D-023).** Both UK GDPR duties built. Erasure on request (Art.17): the admin "Erase lead" action hard-deletes a `human_handoff` row; full erasure stays a documented two-part procedure (DB delete **plus** Mark deleting the matching escalation email; verify requester identity first, Art.12(6)). Storage limitation (Art.5(1)(e)): a daily Netlify scheduled function (`server/netlify/functions/purge-handoffs.js`) deletes `human_handoff` rows older than `shared/config/retention.js` (6 months, the period the privacy notice states). Remaining: the DP review confirms the period (a one-line change if it moves), and the schedule's live registration is checked on the Netlify deploy.
- [ ] **Turn on review requests (D-025).** The one-click review flow is built, merged and live but **dormant**. Exact switch-on instructions (accounts, env vars, redeploy, smoke test) are in **[docs/REVIEW_REQUESTS_SETUP.md](docs/REVIEW_REQUESTS_SETUP.md)**. In short: set **`GOOGLE_REVIEW_URL`** (Mark's Google Business Profile review link — a short `g.page/r/...` keeps the SMS to one segment), **`SMSWORKS_API_KEY`** (from Mark's The SMS Works account, D-009), optionally **`SMS_SENDER_ID`** (default `ICCleaning`) in Netlify; email also relies on the A4 vars (`CUSTOMER_FROM`, `CUSTOMER_REPLY_TO`, `PUBLIC_SITE_URL`). **Then trigger a redeploy** — Netlify functions pick up new env vars only on the next deploy, not the moment the var is saved.
- [ ] **Remove the pre-launch `noindex` at cutover.** `netlify.toml` serves a site-wide `X-Robots-Tag: noindex` while the site is on the `.netlify.app` host and every canonical points at a domain that currently serves a 123 Reg parking page. Flagged `TODO(prelaunch/noindex)`. **Leaving it in place after the domain cutover keeps the real site out of the index entirely**, so it is the first thing to check if nothing appears in Search Console. Leave `robots.txt` alone: it must keep `Allow` for crawlers to see the header at all.
- [ ] **Flip the GitHub repo to private at launch.** `randommonicle/icc-site` is public today (deliberately, while pre-launch). No credentials are exposed — `.env` is git-ignored and has never appeared in a commit — but pricing logic, Mark's contact details and the full decision history are readable by anyone. Flip it with the domain cutover: `gh repo edit randommonicle/icc-site --visibility private`.
- [ ] **Add a privacy notice** to the public site before collecting data at any scale. Drafted and linked across the site; the ICO registration number (ZC230232) is now in the Astro privacy page (`site/src/pages/privacy.astro`). Remaining before go-live: the international-transfer `[to confirm: …]` position and a data-protection review ([docs/DESIGN.md](docs/DESIGN.md) §11).
...
- **The client:** Mark McClymont, Intelligent Carpet Cleaning, Cheltenham — phone 01452 452356 (ICC's own line, D-030; 01242 279590 is Regency's), email hello@intelligentclean.co.uk.

...
- `cf3dec7` docs: D-047 drafted for Ben, D-021's deferral note pointed at it, ROADMAP Phase 2 lines, the CLAUDE.md Supabase row and tree.
- `41c1cc9` cross-agent review round 1 (GPT via codex, `exchange/REVIEW_job-photos_2026-09-19.md`): four findings reproduced and fixed. The order REVERSED to row-first (a deadline cannot cancel a call, so the only reachable inconsistent state must be the harmless one); ONE 5 s deadline for the whole call with cleanup never awaited and no new step after it fires; late results tidy their own row; signing bounded; the admin guard tightened to the exact signed prefix; the migration made corrective (`on conflict do update`, proven on the local stack); `scripts/erase-job.js` + `test/erase-job.test.js` as the real erasure caller (refuses invoiced jobs; customer only when unreferenced; integration-proven).
- `4d951c8` review round 2: an upload error is not proof nothing landed (storage-js reports a committed object with an unparsable success body as an error), so the undo removes the object first (a no-op for a missing key, measured) and drops the row only when that succeeded. The erasure check/delete window accepted and recorded in the script header. Round 3: `[[CONVERGED]]`.
- (this commit) docs: D-047 rules 3 and 5 and its verification rewritten to the reviewed shape; this log closed below.
...
- [x] Cross-agent review (GPT via codex, three rounds, converged): five findings fixed, one residual accepted; the transcript is machine-local in `exchange/`.

...
- **Phase 6 — DONE + DEPLOYED (2026-09-03).** pgTAP done: dropped the stale `jobs_trading_hours` assertion,
  ...
  `feat/d027-per-day-hours`: `667ed29`, `8efd081`, `7f09775`, `e4a1030`, plus `abeb2b5` (review fixes:
  Resend idempotency key + an enforcing concurrent-stale-reclaim test). The cross-agent review converged
  (CLAUDE + GEMINI; `agent-exchange/REVIEW_d027-strict-retry_2026-09-03.md`); migration `20260903120000` was
  applied to prod and catalog-verified; the branch merged to `main` (`2f8a0ff`) and deployed (Netlify
  production `ready`). Only the one real end-to-end ride remains, as the first post-deploy check.
